...
let AllTypes: Types[] = [
  ...
  {
    name: "Meat & Fish",
    imageUrl: "https://cdn-icons-png.flaticon.com/512/3082/3082005.png",
    price: 200,
    rating: 4,
  },
  {
...
const Shopright = () => {
  return (
    <div className="w-[80%] px-10">
      <div className="grid grid-cols-4 grow gap-6">
        {AllTypes.map((data, index) => (
          <div
            key={index}
            className="flex border hover:border-green-500 h-60 w-48 flex-col hover:scale-105 justify-center items-center py-2 shadow-lg rounded-md relative group"
          >
            <div className="relative w-40 h-40">
              <Image
                ...
                alt=""
              />
              <div className="bg-gray-300 rounded-full p-1.5 absolute right-0 top-0 opacity-0 group-hover:opacity-100 transition-opacity">
                <CiHeart size={20} />
              </div>
            </div>
            <p className="text-xs w-full text-start px-2 mt-2 text-black">
              {data.name}
            </p>
            <p className="text-xs w-full text-start px-2 mt-1 font-semibold text-black">
              Rs {data.price}
            </p>
            <div className="bg-gray-300 rounded-full p-1.5 hover:text-white hover:bg-green-400 absolute right-3 bottom-3">
